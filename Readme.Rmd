...
In this project we will read and analyze dataset [Diabetes health indicator dataset](https://www.kaggle.com/datasets/alexteboul/diabetes-health-indicators-dataset/) for each education level. We will create different classification models for predicting the `Diabetes_binary` variable. The best model will be choosen based on `log loss` function.

This report is built for education level `r params$education_level`.

# Packages

In order to achive our goals, we will be using the following `R` packages.
